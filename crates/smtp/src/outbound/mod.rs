...
impl Status<HostResponse<String>, ErrorDetails> {
    ...
    pub fn from_mta_sts_error(entity: &str, err: mta_sts::Error) -> Self {
        match &err {
            mta_sts::Error::Dns { source, .. } => {
                if source.contains("DnsRecordNotFound") {
                    Status::PermanentFailure(ErrorDetails {
                        entity: entity.to_string(),
                        details: Error::MtaStsError(format!("Record not found: {}", source)),
                    })
                } else if source.contains("InvalidRecordType") {
                    Status::PermanentFailure(ErrorDetails {
                        entity: entity.to_string(),
                    ...
                    Status::TemporaryFailure(ErrorDetails {
                        entity: entity.to_string(),
                        details: Error::MtaStsError(format!("DNS lookup error: {}", source)),
                    })
                }
            }
            mta_sts::Error::Http { source, status_code, .. } => {
                if source.contains("timeout") {
                    Status::TemporaryFailure(ErrorDetails {
                        entity: entity.to_string(),
                        details: Error::MtaStsError("Timeout fetching policy.".into()),
                    })
                } else if source.contains("connect") {
                    Status::TemporaryFailure(ErrorDetails {
                        entity: entity.to_string(),
            ...
            mta_sts::Error::InvalidPolicy { reason, .. } => Status::PermanentFailure(ErrorDetails {
                entity: entity.to_string(),
                details: Error::MtaStsError(format!("Failed to parse policy: {}", reason)),
            }),
            mta_sts::Error::PolicyTooLarge { actual_size, max_size } => Status::PermanentFailure(ErrorDetails {
                entity: entity.to_string(),
                details: Error::MtaStsError(format!("Policy too large: {} bytes (max: {})", actual_size, max_size)),
            }),
            mta_sts::Error::Timeout { operation, .. } => Status::TemporaryFailure(ErrorDetails {
                entity: entity.to_string(),
                details: Error::MtaStsError(format!("Operation '{}' timed out", operation)),
            }),
            mta_sts::Error::RateLimited { domain, .. } => Status::TemporaryFailure(ErrorDetails {
                entity: entity.to_string(),
                details: Error::MtaStsError(format!("Rate limited for domain '{}'", domain)),
            }),
            mta_sts::Error::Cache { operation, source } => Status::TemporaryFailure(ErrorDetails {
                entity: entity.to_string(),
                details: Error::MtaStsError(format!("Cache operation '{}' failed: {}", operation, source)),
            }),
        }
    }
}
