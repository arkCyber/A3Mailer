                    ...
                    .await
                {
                    ...
                    Err(err) => {
                        ...
                        if let Some(tls_report) = &tls_report {
                            match &err {
                                mta_sts::Error::Dns { source, .. } if source.contains("DnsRecordNotFound") => {
                                    if strict {
                                ...
                                mta_sts::Error::Dns { source, .. } if source.contains("DnsError") => (),
                                _ => {
                                    server
                        ...
                        match &err {
                            ...
                            mta_sts::Error::PolicyTooLarge { actual_size, max_size } => {
                                trc::event!(
                                    MtaSts(MtaStsEvent::PolicyFetchError),
                                    SpanId = message.span_id,
                                    Domain = domain.to_string(),
                                    Reason = format!("Policy too large: {} bytes (max: {})", actual_size, max_size),
                                    Strict = strict,
                                    Elapsed = time.elapsed(),
                                );
                            }
                            mta_sts::Error::Timeout { operation, timeout, elapsed } => {
                                trc::event!(
                                    MtaSts(MtaStsEvent::PolicyFetchError),
                                    SpanId = message.span_id,
                                    Domain = domain.to_string(),
                                    Reason = format!("Operation '{}' timed out after {:?} (limit: {:?})", operation, elapsed, timeout),
                                    Strict = strict,
                                    Elapsed = time.elapsed(),
                                );
                            }
                            mta_sts::Error::RateLimited { domain: rate_domain, retry_after } => {
                                trc::event!(
                                    MtaSts(MtaStsEvent::PolicyFetchError),
                                    SpanId = message.span_id,
                                    Domain = domain.to_string(),
                                    Reason = format!("Rate limited for domain '{}', retry after {:?}", rate_domain, retry_after),
                                    Strict = strict,
                                    Elapsed = time.elapsed(),
                                );
                            }
                            mta_sts::Error::Cache { operation, source } => {
                                trc::event!(
                                    MtaSts(MtaStsEvent::PolicyFetchError),
                                    SpanId = message.span_id,
                                    Domain = domain.to_string(),
                                    Reason = format!("Cache operation '{}' failed: {}", operation, source),
                                    Strict = strict,
                                    Elapsed = time.elapsed(),
                                );
                            }
