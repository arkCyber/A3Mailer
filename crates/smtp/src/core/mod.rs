...
impl Session<common::listener::stream::NullIo> {
    ...
    /// Checks if the last operation failed and returns the error response if any
    ///
    /// # Returns
    /// * `None` - If the operation succeeded (response starts with '2')
    /// * `Some(String)` - The error response message if the operation failed
    pub fn has_failed(&mut self) -> Option<String> {
        if self.stream.tx_buf.first().is_none_or(|&c| c == b'2') {
            self.stream.tx_buf.clear();
            None
        } else {
            // Safely convert bytes to UTF-8, handling invalid UTF-8 gracefully
            let response = match std::str::from_utf8(&self.stream.tx_buf) {
                Ok(s) => s.trim().to_string(),
                Err(_) => {
                    // Log the invalid UTF-8 and return a generic error message
                    trc::event!(
                        Smtp(trc::SmtpEvent::Error),
                        Details = "Invalid UTF-8 in SMTP response buffer",
                        Size = self.stream.tx_buf.len()
                    );
