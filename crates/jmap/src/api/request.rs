        ...
        for mut call in request.method_calls {
            ...
            loop {
                ...
                if let Some(next_call) = next_call {
                    call = next_call;
                    // Safely get the last response ID, or use a default if none exists
                    if let Some(last_response) = response.method_responses.last() {
                        call.id.clone_from(&last_response.id);
                    } else {
                        // This should not happen in normal operation, but handle gracefully
                        trc::event!(
                            Jmap(trc::JmapEvent::InvalidResultReference),
                            Details = "No method responses available when processing next call"
                        );
                        // Keep the original call ID as fallback
                    }
                } else {
                    break;
                }
