...
mod tests {
    ...
    #[test]
    fn test_propfind_request_handler_trait() {
        let start_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();
        info!("Starting PROPFIND request handler trait test at {:?}", start_time);

        // Test that the trait is properly defined
        // This is a compile-time test to ensure the trait signature is correct
        fn assert_trait_implemented<T: PropFindRequestHandler>(_: T) {}

        // If this compiles, the trait is properly defined
        debug!("PROPFIND request handler trait test completed successfully");
    }

    #[test]
    fn test_propfind_depth_values() {
        let start_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();
        info!("Starting PROPFIND depth values test at {:?}", start_time);

        // Test depth values used in PROPFIND
        ...
        debug!("PROPFIND depth values test completed successfully");
    }

    #[test]
    fn test_propfind_property_types() {
        let start_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();
        info!("Starting PROPFIND property types test at {:?}", start_time);

        // Test different property types
        let webdav_prop = DavProperty::WebDav(WebDavProperty::DisplayName);
        let caldav_prop = DavProperty::CalDav(CalDavProperty::CalendarDescription);
        let carddav_prop = DavProperty::CardDav(CardDavProperty::AddressbookDescription);
        let principal_prop = DavProperty::Principal(PrincipalProperty::DisplayName);

        assert!(matches!(webdav_prop, DavProperty::WebDav(_)));
        assert!(matches!(caldav_prop, DavProperty::CalDav(_)));
        assert!(matches!(carddav_prop, DavProperty::CardDav(_)));
        assert!(matches!(principal_prop, DavProperty::Principal(_)));

        debug!("PROPFIND property types test completed successfully");
    }

    #[test]
    fn test_propfind_response_structure() {
        let start_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();
        info!("Starting PROPFIND response structure test at {:?}", start_time);

        // Test MultiStatus response structure
        let mut response = MultiStatus::new();

        // Test that we can add responses
        let prop_stat = PropStat::new_list(vec![]);
        let dav_response = Response::new_propstat("/test/", vec![prop_stat]);
        response.add_response(dav_response);

        // Verify response was added
        assert!(!response.responses.is_empty());

        debug!("PROPFIND response structure test completed successfully");
    }

    #[test]
    fn test_propfind_status_codes() {
        let start_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();
        info!("Starting PROPFIND status codes test at {:?}", start_time);

        // Test status codes used in PROPFIND responses
        ...
        debug!("PROPFIND status codes test completed successfully");
    }

    #[test]
    fn test_propfind_namespaces() {
        let start_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();
        info!("Starting PROPFIND namespaces test at {:?}", start_time);

        // Test namespace handling
        let dav_namespace = Namespace::Dav;
        let caldav_namespace = Namespace::CalDav;
        let carddav_namespace = Namespace::CardDav;

        assert_eq!(format!("{:?}", dav_namespace), "Dav");
        assert_eq!(format!("{:?}", caldav_namespace), "CalDav");
        assert_eq!(format!("{:?}", carddav_namespace), "CardDav");

        debug!("PROPFIND namespaces test completed successfully");
    }

    #[test]
    fn test_propfind_resource_types() {
        let start_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();
        info!("Starting PROPFIND resource types test at {:?}", start_time);

        // Test resource types
        let collection = ResourceType::Collection;
        let calendar = ResourceType::Calendar;
        let addressbook = ResourceType::AddressBook;
        let principal = ResourceType::Principal;

        assert_eq!(format!("{:?}", collection), "Collection");
        assert_eq!(format!("{:?}", calendar), "Calendar");
        assert_eq!(format!("{:?}", addressbook), "AddressBook");
        assert_eq!(format!("{:?}", principal), "Principal");

        debug!("PROPFIND resource types test completed successfully");
    }

    #[test]
    fn test_propfind_etag_handling() {
        let start_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();
        info!("Starting PROPFIND ETag handling test at {:?}", start_time);
        ...
        debug!("PROPFIND ETag handling test completed successfully");
    }

    #[test]
    fn test_propfind_privileges() {
        let start_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();
        info!("Starting PROPFIND privileges test at {:?}", start_time);
        ...
        debug!("PROPFIND privileges test completed successfully");
    }

    #[test]
    fn test_propfind_supported_locks() {
        let start_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();
        info!("Starting PROPFIND supported locks test at {:?}", start_time);

        // Test supported lock types
        let exclusive_lock = SupportedLock::Exclusive;
        let shared_lock = SupportedLock::Shared;

        assert_eq!(format!("{:?}", exclusive_lock), "Exclusive");
        assert_eq!(format!("{:?}", shared_lock), "Shared");

        debug!("PROPFIND supported locks test completed successfully");
    }

    #[test]
    fn test_propfind_report_set() {
        let start_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();
        info!("Starting PROPFIND report set test at {:?}", start_time);

        // Test report set structure
        let report_set = ReportSet::new();

        // Verify report set can be created
        assert!(!format!("{:?}", report_set).is_empty());

        debug!("PROPFIND report set test completed successfully");
    }

    #[test]
    fn test_propfind_collation_support() {
        let start_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();
        info!("Starting PROPFIND collation support test at {:?}", start_time);

        // Test collation types
        let ascii_collation = Collation::AsciiCasemap;
        let unicode_collation = Collation::UnicodeCollation;

        assert_eq!(format!("{:?}", ascii_collation), "AsciiCasemap");
        assert_eq!(format!("{:?}", unicode_collation), "UnicodeCollation");

        debug!("PROPFIND collation support test completed successfully");
    }

    #[test]
    fn test_propfind_href_structure() {
        let start_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();
        info!("Starting PROPFIND Href structure test at {:?}", start_time);

        // Test Href structure
        let href = Href("/calendar/user/personal/".to_string());
        assert_eq!(href.0, "/calendar/user/personal/");

        let href_with_domain = Href("https://example.com/dav/calendar/".to_string());
        assert!(href_with_domain.0.starts_with("https://"));

        debug!("PROPFIND Href structure test completed successfully");
    }

    #[test]
    fn test_propfind_dead_properties() {
        let start_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();
        info!("Starting PROPFIND dead properties test at {:?}", start_time);

        // Test dead property structure
        let dead_prop = DeadProperty {
            name: "custom-property".to_string(),
            namespace: Some("http://example.com/ns".to_string()),
            value: "custom-value".to_string(),
        };

        assert_eq!(dead_prop.name, "custom-property");
        assert!(dead_prop.namespace.is_some());
        assert_eq!(dead_prop.value, "custom-value");

        debug!("PROPFIND dead properties test completed successfully");
    }
}
