    ...
    async fn handle_api_manage_request(
        ...
        match path.first().copied().unwrap_or_default() {
            ...
            "telemetry" => {
                ...
                // and substantial financial penalties. A3Mailer Team LLC actively monitors for
    ...
    fn unauthorized(include_realms: bool) -> Self {
        (if include_realms {
            HttpResponse::new(StatusCode::UNAUTHORIZED)
                .with_header(header::WWW_AUTHENTICATE, "Bearer realm=\"A3Mailer Server\"")
                .with_header(header::WWW_AUTHENTICATE, "Basic realm=\"A3Mailer Server\"")
        } else {
            HttpResponse::new(StatusCode::UNAUTHORIZED)
        })
