...
impl<T: SessionStream> Session<T> {
    ...
    pub async fn into_tls(self) -> Result<Session<TlsStream<T>>, ()> {
        ...
        // Safely replace the stream in the state
        let new_state = state.try_replace_stream_tx(stream_tx.clone()).ok_or_else(|| {
            trc::event!(
                Network(trc::NetworkEvent::SplitError),
                SpanId = self.session_id,
                Details = "Failed to replace stream in state after TLS upgrade"
            );
        })?;
