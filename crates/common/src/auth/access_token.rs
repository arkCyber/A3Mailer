...
//! Access token management with comprehensive error handling and logging
//!
//! This module provides production-grade access token creation, validation,
//! and management with proper error handling, detailed logging, and security
//! features for the Stalwart mail server.
...
mod tests {
    ...
    fn create_test_principal() -> Principal {
        Principal::new(1, Type::Individual)
    }

    /// Test PrincipalOrId enum
    #[test]
    fn test_principal_or_id() {
        let principal = create_test_principal();
        let principal_or_id = PrincipalOrId::Principal(principal);

        match principal_or_id {
            PrincipalOrId::Principal(p) => {
                assert_eq!(p.id(), 1);
                assert_eq!(p.typ(), Type::Individual);
            }
            PrincipalOrId::Id(_) => panic!("Expected Principal variant"),
        }

        let id_variant = PrincipalOrId::Id(42);
        match id_variant {
            PrincipalOrId::Id(id) => assert_eq!(id, 42),
            PrincipalOrId::Principal(_) => panic!("Expected Id variant"),
        }
    }
    ...
    fn test_basic_functionality() {
        // Test that we can create a principal
        let principal = create_test_principal();
        assert_eq!(principal.id(), 1);
        assert_eq!(principal.typ(), Type::Individual);
    }
    ...
    fn test_directory_types() {
        ...
        for typ in types {
            let principal = Principal::new(1, typ);
            assert_eq!(principal.typ(), typ);
        }
    }
}
