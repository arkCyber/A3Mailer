                ...
                if let Some(span) = &event.inner.span {
                    ...
                    if !event.inner.typ.is_span_end() {
                        ...
                        if !queue_ids.is_empty() {
                            // Serialize events
                            #[cfg(feature = "enterprise")]
                            let serialized_data = serialize_events(
                                [span.as_ref()]
                                    .into_iter()
                            ...
                            let serialized_data = Vec::new(); // Placeholder for non-enterprise
                            ...
                            batch.set(
                                ValueClass::Telemetry(TelemetryClass::Index {
                                    span_id,
                                    value: (span.inner.typ.id() as u16).to_be_bytes().to_vec(),
                                }),
                                vec![],
                            );
        ...
        for (param_num, param) in params.iter().enumerate() {
            let (value, exact_len) = match param {
                TracingQuery::EventType(event) => (
                    (event.id() as u16).to_be_bytes().to_vec(),
...
impl Deserialize for Span {
    fn deserialize(bytes: &[u8]) -> trc::Result<Self> {
        ...
        #[cfg(not(feature = "enterprise"))]
        {
            // For non-enterprise, return empty span
            Ok(Self(Vec::new()))
        }
    }
}
