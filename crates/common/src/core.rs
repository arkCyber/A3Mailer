...
//!
//! This module provides the core server functionality for A3Mailer Mail Server.
//! It implements the main server structure and provides access to various
//! storage backends, authentication systems, and configuration management.
//!
//! ## Key Components
//!
//! - **Storage Access**: Provides unified access to data stores, blob stores, and FTS
//! - **Directory Services**: Manages user directories and authentication
//! - **Queue Management**: Handles SMTP queue operations and strategies
//! - **Resource Management**: Manages quotas, tokens, and resource allocation
//! - **State Management**: Handles state changes and broadcasting
//!
//! ## Performance Considerations
//!
//! This module is designed for high-performance operation with:
...
//! ## Error Handling
//!
//! All operations return `trc::Result<T>` for comprehensive error tracking
...
impl Server {
    /// Returns a reference to the main data store.
    ///
    /// This is the primary storage backend for all persistent data including
    /// emails, metadata, and configuration. The store provides ACID transactions
    /// and is optimized for high-throughput operations.
    ///
    /// # Performance Notes
    ///
    /// This method is inlined for optimal performance in hot paths.
    #[inline(always)]
    pub fn store(&self) -> &Store {
        &self.core.storage.data
    }

    /// Returns a reference to the blob storage backend.
    ///
    /// The blob store is optimized for storing large binary objects such as
    /// email attachments, images, and other file data. It supports deduplication
    /// and efficient streaming operations.
    ///
    /// # Performance Notes
    ///
    /// This method is inlined for optimal performance in hot paths.
    ...
    /// Returns a reference to the full-text search store.
    ///
    /// The FTS store provides indexed search capabilities across email content,
    /// headers, and metadata. It supports complex queries and ranking algorithms.
    ///
    /// # Performance Notes
    ///
    /// This method is inlined for optimal performance in hot paths.
    ...
    ///
    /// The in-memory store is used for caching frequently accessed data,
    /// temporary storage, and high-speed lookups. Data in this store is
    /// volatile and may be lost on restart.
    ///
    /// # Performance Notes
    ///
    /// This method is inlined for optimal performance in hot paths.
    ...
    /// Returns a reference to the default directory service.
    ///
    /// The directory service manages user accounts, authentication,
    /// and authorization. It provides a unified interface to various
    /// backend directory systems (LDAP, SQL, etc.).
    ///
    /// # Performance Notes
    ///
    /// This method is inlined for optimal performance in hot paths.
    ...
    /// Retrieves a named directory service.
    ///
    /// Returns a reference to the specified directory service if it exists.
    /// This allows for multiple directory backends to be configured and
    /// accessed by name.
    ///
    /// # Arguments
    ///
    /// * `name` - The name of the directory service to retrieve
    ///
    /// # Returns
    ///
    /// * `Some(&Arc<Directory>)` - If the named directory exists
    /// * `None` - If the named directory does not exist
    ///
    /// # Examples
    ///
    /// ```rust,ignore
    /// if let Some(ldap_dir) = server.get_directory("ldap") {
    ///     // Use LDAP directory
    /// }
    /// ```
    ...
    /// Retrieves a named directory service or returns the default.
    ///
    /// This method ensures that a directory service is always returned,
    /// falling back to the default directory if the named one doesn't exist.
    /// It also logs when a requested directory is not found for debugging.
    ///
    /// # Arguments
    ///
    /// * `name` - The name of the directory service to retrieve
    ...
    /// # Returns
    ///
    /// Always returns a valid directory reference, either the named one
    /// or the default directory service.
    ///
    /// # Logging
    ///
    /// Logs a `DirectoryNotFound` event when the named directory doesn't exist.
    ...
    /// Retrieves a named in-memory store.
    ///
    /// In-memory stores are used for caching, temporary data, and high-speed
    /// lookups. Each named store can have different configurations and purposes.
    ///
    /// # Arguments
    ///
    /// * `name` - The name of the in-memory store to retrieve
    ///
    /// # Returns
    ///
    /// * `Some(&InMemoryStore)` - If the named store exists
    /// * `None` - If the named store does not exist
    ///
    /// # Performance Notes
    ///
    /// In-memory stores provide the fastest access times but are volatile.
    pub fn get_in_memory_store(&self, name: &str) -> Option<&InMemoryStore> {
        self.core.storage.lookups.get(name)
    }

    /// Retrieves a named in-memory store or returns the default.
    ///
    /// This method ensures that an in-memory store is always returned,
    /// falling back to the default store if the named one doesn't exist.
    ///
    /// # Arguments
    ///
    /// * `name` - The name of the in-memory store to retrieve
    ...
    /// # Returns
    ///
    /// Always returns a valid in-memory store reference.
    ///
    /// # Logging
    ///
    /// Logs a `StoreNotFound` event when the named store doesn't exist.
    ...
    /// Retrieves a named data store or returns the default.
    ///
    /// Data stores provide persistent storage with ACID properties.
    /// Multiple named stores can be configured for different purposes
    /// (e.g., separate stores for different tenants or data types).
    ///
    /// # Arguments
    ///
    /// * `name` - The name of the data store to retrieve
    /// * `session_id` - Session ID for logging and tracing
    ///
    /// # Returns
    ///
    /// Always returns a valid data store reference.
    ///
    /// # Logging
    ///
    /// Logs a `StoreNotFound` event when the named store doesn't exist.
