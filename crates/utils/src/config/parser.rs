...
// Simple TOML parser for A3Mailer Server configuration files.
impl Config {
    ...
    fn value(&mut self, key: String, stop_chars: &[char], nest_level: usize) -> Result<char> {
        ...
        match self.next_char(true, false)? {
            ...
            '!' => {
                ...
                while let Some(ch) = self.iter.peek() {
                    if ch.is_alphanumeric() || ['_'].contains(ch) {
                        env_var.push(self.next_char(true, false)?);
                    } else {
                        break;
                    }
                }

                // Get environment variable value or use empty string as default
